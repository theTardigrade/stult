...
- [Map merge](../examples/tests/map_merge.stult)  
  Checks that `STD.TYPE.MAP.MERGE` returns a new mutable shallow merge by default and recursively merges nested maps when passed `+`, while accepting frozen inputs, preserving winning entry mutability, reusing values where appropriate and rejecting invalid arguments.
