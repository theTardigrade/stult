...
Bindings may also carry optional runtime type contracts. `<.>` records the runtime value kind assigned when the binding is created and rejects later assignments of a different kind. `<*>` explicitly requests the default dynamic behaviour. Named contracts such as `STD.TYPE.VOID`, `STD.TYPE.NUMBER`, `STD.TYPE.COLLECTION`, `STD.TYPE.ARRAY<STD.TYPE.STRING>` and `STD.TYPE.MAP<STD.TYPE.BOOL>` are parsed as contract descriptors. `STD.TYPE.COLLECTION` accepts arrays, maps and strings and behaves like `STD.TYPE.ARRAY<*>|STD.TYPE.MAP<*>|STD.TYPE.STRING`; it does not accept a nested contract. Union contracts such as `STD.TYPE.NUMBER|STD.TYPE.BOOL` accept any value that satisfies one of their options. Contract literal expressions, such as `<STD.TYPE.NUMBER>`, produce contract values whose runtime kind is accepted by `STD.TYPE.CONTRACT`. A binding whose current value is a contract value may be used as a contract alias in later contract positions. Contracts are enforced when the binding is created and on later reassignment. Map literal entries do not have their own angle-bracket contract syntax; homogeneous map value contracts are expressed with `STD.TYPE.MAP<contract>` on the binding that receives the map. Structured map contracts are expressed with `STD.TYPE.MAP<{...}>` and can declare required keys, optional keys and an optional `_` wildcard for other keys. They are strict by default, so extra keys are rejected unless a wildcard is present. Function parameters may also carry contracts, optional contracted parameters write the optional marker before the contract as `name?<contract>`, function literals may declare return contracts after the parameter list, and function signature contracts are expressed with `STD.TYPE.FUNCTION<(parameter-contracts): return-contract>`.

Collection contracts also attach to the collection value itself. An array value created or assigned through `STD.TYPE.ARRAY<contract>` stores the element contract on the `Array`, a homogeneous map contract created through `STD.TYPE.MAP<contract>` stores the value contract on the `Map`, and a structured map contract created through `STD.TYPE.MAP<{...}>` stores the structured contract on the `Map`. Mutation paths such as array index assignment, map index assignment and helper functions that call `Array.Set` or `Map.Set` must check those collection-level contracts. This prevents aliases from bypassing the contract. Nested contracts are recursive, and the nested contract position may contain another named contract, a union contract, a contract alias, `<.>` or `<*>`. Unresolved nested `<.>` contracts learn their value kind from existing collection contents or from the first later mutation. Function signature contracts attach to the function value; call paths check signature arguments before the function body runs and check the return value after normal or early return. When a function with inner parameter or return contracts is assigned to a signature contract, those inner contracts are checked for compatibility immediately.

Contract markers are declaration syntax: they may appear only when a binding is created, not on later reassignment. Because bytecode execution needs to distinguish explicit `<*>` from an uncontracted assignment, the compiler emits separate store opcodes for explicit-any contracts even though the resulting binding contract is the same as the default any-kind contract. Named and union contracts are stored directly on bytecode instructions so the VM can create and validate the same runtime contract as the interpreter. Contracts that contain aliases are resolved against the current runtime bindings when the contract literal is evaluated or when a contract declaration is attached to a new binding.
