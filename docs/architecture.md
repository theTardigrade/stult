...
Try-catch has both syntax and runtime implications. Parser changes should preserve the touching `'{` opener and the pipe-based `}|{` separator. The separator is written touching idiomatically, but horizontal whitespace around the pipe is accepted on the same line. Runtime changes should keep break and early return separate from catchable errors. In bytecode, any control-flow path that leaves a protected try region must also emit matching `TRY_END` instructions.
